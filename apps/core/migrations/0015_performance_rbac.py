from django.db import migrations

_PERMS = [
    ('performance.view', 'performance', 'View performance'),
    ('performance.manage', 'performance', 'Manage performance'),
]
_FULL_ACCESS = ('super_admin', 'company_admin', 'internal_hr', 'deployed_hr', 'hr')
_VIEW_ONLY = ('internal_manager', 'deployed_manager', 'manager',
              'white_collar_employee', 'blue_collar_employee', 'employee')


def add_perms(apps, schema_editor):
    Role = apps.get_model('core', 'Role')
    Permission = apps.get_model('core', 'Permission')
    RolePermission = apps.get_model('core', 'RolePermission')

    perms = []
    for codename, module, description in _PERMS:
        perm, _ = Permission.objects.get_or_create(
            codename=codename,
            defaults={'module': module, 'description': description})
        perms.append(perm)

    view_perm = next(p for p in perms if p.codename == 'performance.view')

    for slug in _FULL_ACCESS:
        for role in Role.objects.filter(slug=slug, company_id=None):
            for perm in perms:
                RolePermission.objects.get_or_create(role=role, permission=perm)

    for slug in _VIEW_ONLY:
        for role in Role.objects.filter(slug=slug, company_id=None):
            RolePermission.objects.get_or_create(role=role, permission=view_perm)


def noop(apps, schema_editor):
    pass


class Migration(migrations.Migration):
    dependencies = [('core', '0014_lms_rbac')]
    operations = [migrations.RunPython(add_perms, reverse_code=noop)]
